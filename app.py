...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://username:password@databaseaddress/databasename'
db = SQLAlchemy(app)
...
#Define the login credentials and check if a user is logged in
...
def login():
    if request.method == 'POST':
        ...
        # Add your logic to validate username and password
    ...
    return render_template('login.html', translations_selected=translations_selected)
...
#Check Login state before each request and if not logged in, redirect to login page    
@app.before_request
def check_login():
    allowed_routes=['login','static','templates']
